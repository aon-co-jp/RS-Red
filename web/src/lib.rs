...
fn wire_ticket_form() {
    add_click("create-ticket-btn", || {
        wasm_bindgen_futures::spawn_local(async {
            ...
            let body = serde_json::json!({ "title": title, "description": description, "project_id": project_id }).to_string();
...
async fn load_tickets(project_id: u64) {
    ...
    for t in &tickets {
        ...
        html.push_str(&format!(
            r#"<li><button class="link-btn" onclick="open_ticket({id})">{title}</button> <span class="badge">{status}</span></li>"#
        ));
    }
    if html.is_empty() {
